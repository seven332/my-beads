...
test("creates only on request, returns to the current work, and exports without leaving the editor", async ({ page }) => {
  await page.goto("/");
  ...
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem("my-beads.draft")!).grid[0][0])).toBe("H7");
  ...
  await page.getByLabel("Open CSV").setInputFiles({ name: "bad.csv", mimeType: "text/csv", buffer: Buffer.from("#55514C") });
  ...
  expect(parsePatternCsv(await readFile((await (await pending).path())!, "utf8"))).toEqual([["H7", null, null], [null, null, null]]);
...
test("CSV ignores blank-canvas settings and preserves empty borders through export", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("Columns", { exact: true }).fill("20");
  await page.getByLabel("Rows", { exact: true }).fill("30");
  await page.getByLabel("Open CSV").setInputFiles({ name: "Borders.csv", mimeType: "text/csv", buffer: Buffer.from('\uFEFF,,,\r\n,H5,#000000,\r\n,,,\r\n') });
  ...
  expect(parsePatternCsv(await readFile((await (await pending).path())!, "utf8"))).toEqual([
    [null, null, null, null], [null, "H5", "H7", null], [null, null, null, null],
  ]);
});
  ...
  for (const locale of ["en-US", "zh-CN"]) {
    ...
    for (const width of [320, 390, 600, 740, 900]) {
      ...
      const layout = await page.evaluate(() => ({ width: document.documentElement.clientWidth, content: document.documentElement.scrollWidth,
        cards: [...document.querySelectorAll(".creation-card")].map(node => node.scrollWidth <= node.clientWidth) }));
