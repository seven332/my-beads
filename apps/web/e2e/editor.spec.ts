...
function verifyPixels(buffer: Buffer, grid: PatternGrid, scale: number) {
  ...
  for (let y = 0; y < png.height; y++)
    for (let x = 0; x < png.width; x++) {
      ...
      if (!code) {
        if (png.data[offset + 3] !== 0) mismatch++;
      } else {
...
for (const width of [1440, 390]) {
  test(`palette recommendations can be compared, chosen by keyboard and exported at ${width}px`, async ({
    page,
    browserName,
  }) => {
    ...
    await page
      .getByLabel("Open CSV")
      .setInputFiles({ name: "blank.csv", mimeType: "text/csv", buffer: Buffer.from('\"\",\"\"') });
    ...
    await expect(page.locator(".color-recommendation")).toHaveAccessibleDescription(
      /Closest color.*Preserve chroma/,
    );
    await expect(page.locator(".selected-color strong")).toHaveText("B23");
...
test("Sherma: all tools, grouped undo, CSV round trip and exact PNG/chart downloads", async ({
  ...
  await page.mouse.click(cell(2, 0).x, cell(2, 0).y);
  grid[0][2] = null;
  ...
  await page
    .getByLabel("Open CSV")
    .setInputFiles({ name: "roundtrip.csv", mimeType: "text/csv", buffer: csv });
  ...
  for (const [code, count] of createPattern(grid).counts) {
    ...
    expect(svg).toContain(`>${count} beads</text>`);
  }
  const chart = PNG.sync.read(await download(page, "chart"));
  expect(chart.width).toBe(2400);
  expect(chart.height).toBe(Number(svg.match(/height="(\d+)"/)![1]));
  const sample = (192 + 5) * chart.width + 125 + 5;
  expect([...chart.data.subarray(sample * 4, sample * 4 + 4)]).toEqual([255, 255, 255, 255]);
  expect(errors).toEqual([]);
});

test("zoom and pan preserve cell targeting; invalid imports and exports preserve work", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.mouse.click(center.x + 60 + zoom * 1.25 * 0.5, center.y + 20 + zoom * 1.25 * 0.5);
  await expect(page.getByTestId("counts")).toHaveText("1 bead · 1 color");
  const grid = parsePatternCsv((await download(page, "csv")).toString());
  expect(grid[2][2]).toBe("H7");
  expect(grid.flat().filter(Boolean)).toHaveLength(1);
  await startNew(page);
  await page
    .getByLabel("Open CSV")
    .setInputFiles({ name: "bad.csv", mimeType: "text/csv", buffer: Buffer.from("H7\nH7,H7") });
...
test("keyboard canvas editing and interrupted pointer strokes", async ({ page }) => {
  ...
  await page.mouse.down();
  await page.getByRole("img", { name: "Pattern canvas" }).press("Escape");
  await page.mouse.up();
  await expect(page.getByTestId("counts")).toHaveText("0 beads · 0 colors");
...
test("fast drags paint to the boundary and remain one undo step", async ({ page }) => {
  ...
  await page.mouse.move(cell(0, 0).x, cell(0, 0).y);
  await page.mouse.down();
  await page.mouse.move(cell(8, 0).x, cell(8, 0).y);
  await page.mouse.up();
...
for (const tool of ["Pencil", "Eraser"] as const) {
  test(`${tool} commits when capture is lost at release before pointerup`, async ({ page }) => {
    ...
    const edited: PatternGrid = [
      [...Array<string | null>(3).fill(tool === "Pencil" ? "H7" : null), "H2"],
    ];
    await page
      .getByLabel("Open CSV")
      .setInputFiles({
    ...
    await expect(page.getByTestId("counts")).toHaveText(
      tool === "Pencil" ? "4 beads · 2 colors" : "1 bead · 1 color",
    );
...
test("drawing on a full-window canvas preserves the fixed page and cell targeting", async ({
  page,
}) => {
