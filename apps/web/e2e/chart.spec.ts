...
async function checkChart(page: Page, csv: string, width: number, title: string) {
  ...
  for (const row of grid)
    for (const code of row) if (code) counts.set(code, (counts.get(code) ?? 0) + 1);

  await page.goto("/");
  await page
    .getByLabel("Open CSV")
    .setInputFiles({ name: `${title}.csv`, mimeType: "text/csv", buffer: Buffer.from(csv) });
  ...
  await expect(
    chart.getByText(
      `${grid[0].length} × ${grid.length} grid · ${counts.size} colors · ${beads} beads`,
      { exact: true },
    ),
  ).toHaveCount(1);

  const layout = await chart.evaluate(
    (svg, { columns, rows }) => {
      const page = (svg as SVGSVGElement).viewBox.baseVal;
      const inside = (inner: DOMRect, outer: DOMRect | SVGRect) =>
        inner.x >= outer.x - 0.5 &&
        inner.y >= outer.y - 0.5 &&
      ...
      const overflow = text
        .filter((label) => !inside(label.getBBox(), page))
      ...
      const legends = cards.map((card) => {
        ...
        return {
          ...
          separated: boxes.every(
            (box, i) => i === 0 || box.y >= boxes[i - 1].y + boxes[i - 1].height,
          ),
        };
      });
      ...
      const coordinates = text
        .filter((label) => /^\d+$/.test(label.textContent ?? ""))
        .map((label) => ({
      ...
      row.flatMap((code, x) =>
        code
          ? [
              {
                code,
                color: defaultPalette.colors[code],
                column: x,
                row: y,
                width: 1,
                height: 1,
  ...
  expect(layout.legends.map((legend) => legend.text).sort()).toEqual(
    [...counts]
      .map(([code, count]) => [code, defaultPalette.colors[code], `${count} beads`])
      .sort(),
  );
...
test("narrow printable export keeps all legend entries inside their cards", async ({ page }) => {
  ...
  const grid = Array.from({ length: 4 }, (_, y) =>
    Array.from({ length: 25 }, (_, x) => (x % 4 === y ? colors[x % colors.length] : null)),
  );
