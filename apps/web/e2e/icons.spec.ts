...
for (const width of [1440, 320]) {
  test(`SVG controls keep their names, actions and Canvas through language updates at ${width}px`, async ({ page }) => {
    ...
    for (const [locale, copy] of [["en-US", en], ["zh-CN", zh]] as const) {
      await page.locator(".language-picker select").selectOption(locale);
      for (const name of [copy.tools.pencil, copy.tools.eraser, copy.tools.bucketLabel, copy.tools.eyedropperLabel, copy.tools.pan]) {
        ...
        const rendered = await graphic.evaluate(svg => ({
          namespace: svg.namespaceURI, stroke: getComputedStyle(svg).stroke,
      ...
      await page.getByRole("button", { name: copy.tools.pencil, exact: true }).locator("svg").click();
      ...
      await expect(undo).toBeEnabled();
      await undo.locator("svg").click();
      await expect(page.getByTestId("counts")).toHaveText(locale === "en-US" ? "0 beads · 0 colors" : "0 颗 · 0 色");

      await openPalette(page);
      if (await page.locator(".palette-toggle").isVisible()) {
        await page.getByRole("button", { name: copy.palette.close, exact: true }).locator("svg").click();
      ...
      await page.getByRole("button", { name: copy.export.close, exact: true }).locator("svg").click();
      await expect(exportButton).toBeFocused();
      expect(await page.locator(".pattern-canvas").evaluate((node, previous) => node === previous, canvas)).toBe(true);
