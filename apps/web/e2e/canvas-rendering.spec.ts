...
test("drawing, erasing, filling and history redraw visible cells without moving the viewport", async ({ page }) => {
...
test("keyboard focus, grid and code overlays and resizing redraw without editing", async ({ page }) => {
  ...
  const hasCode = () => canvas.evaluate((node, area) => {
    const element = node as HTMLCanvasElement, rect = element.getBoundingClientRect();
    const ratioX = element.width / rect.width, ratioY = element.height / rect.height;
    const pixels = element.getContext("2d")!.getImageData(Math.floor(area.x * ratioX), Math.floor(area.y * ratioY),
      Math.floor(area.size * ratioX), Math.floor(area.size * ratioY)).data;
    return pixels.some((value, index) => index % 4 === 0 && value > 200 && pixels[index + 1] > 200 && pixels[index + 2] > 200);
  ...
  await expect.poll(() => canvas.evaluate(node => (node as HTMLCanvasElement).width)).not.toBe(before);
  await expect.poll(() => canvas.evaluate(node => (node as HTMLCanvasElement).width ===
    Math.round(node.getBoundingClientRect().width * Math.min(devicePixelRatio, 2)))).toBe(true);
  await expect(page.getByRole("button", { name: "Undo" })).toBeDisabled();
});
