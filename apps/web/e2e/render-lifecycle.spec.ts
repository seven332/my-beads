...
test("unrelated renders preserve a captured stroke, then keep a text selection and native dialog focus", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("Open CSV").setInputFiles({ name: "Stable.csv", mimeType: "text/csv", buffer: Buffer.from(",,") });
  const canvas = page.locator(".pattern-canvas"), original = await canvas.elementHandle();
  ...
  expect(await canvas.evaluate(element => element.hasPointerCapture(Number(element.getAttribute("data-pointer"))))).toBe(true);
  await page.mouse.move(cell(1, 0).x, cell(1, 0).y); await page.mouse.up();
  ...
  await page.getByRole("button", { name: "Grid", exact: true }).evaluate(element => (element as HTMLButtonElement).click());
  await expect(title).toBeFocused();
  expect(await title.evaluate(element => [(element as HTMLInputElement).selectionStart, (element as HTMLInputElement).selectionEnd])).toEqual([2, 5]);
