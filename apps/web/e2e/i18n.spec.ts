...
async function download(page: Page, label: string): Promise<Buffer> {
  ...
  await page
    .getByRole("button", { name: label === "下载" ? "返回编辑" : "Back to editing", exact: true })
    .click();
  return result;
}
...
test("imports images with Chinese controls and validation, then exports the mapped pixels", async ({
  page,
}) => {
...
test("keeps language controls and color recommendations within narrow viewports in both languages", async ({
  page,
}) => {
  await page.goto("/");
