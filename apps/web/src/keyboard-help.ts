...
export function keyboardHelp(
  ...
  return modal(
    {
      ref: dialog,
      labelledBy: "keyboard-heading",
      className:
        "keyboard-dialog max-h-[calc(100dvh_-_24px)] w-[min(720px,calc(100vw_-_24px))] overscroll-contain p-6 mobile:p-4",
      onCancel: close,
    },
    html`<div class="keyboard-heading flex items-center justify-between gap-3">
        <h2 class="text-[22px]" id="keyboard-heading">${t(($) => $.keyboard.heading)}</h2>
        ${iconButton(
          t(($) => $.keyboard.close),
          X,
          { className: "icon-button", onClick: close },
        )}
      </div>
