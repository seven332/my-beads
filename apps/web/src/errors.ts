...
export class UiError extends Error {
  constructor(readonly code: ErrorCode, readonly values: Readonly<Record<string, string | number>> = {}) {
...
export function errorText(error: unknown, t: Translate): string {
  ...
  if (error instanceof BeadError || error instanceof UiError) return t($ => $.errors[error.code], { replace: error.values });
  return t($ => $.errors.unexpected, { detail: error instanceof Error ? error.message : String(error) });
}
