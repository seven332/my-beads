...
export function mountApp(
  ...
  function closeExport() {
    ...
    store.set(exports.closeExport$);
    if (wasOpen)
      host
        .querySelector<HTMLButtonElement>(".document-actions .primary")
        ?.focus({ preventScroll: true });
  }
  function flushDraft() {
    drafts.observe(store.get(state.committedDocument$));
    drafts.flush();
  }
  function fit() {
  ...
  function palette(open: boolean) {
    ...
    const entry =
      store.get(state.editor$).paletteView === "all"
        ? ".palette-search"
        : '.palette-view button[aria-pressed="true"]';
    host
      .querySelector<HTMLElement>(open ? entry : ".palette-toggle")
      ?.focus({ preventScroll: true });
  }
  ...
  const actions: Actions = {
    ...
    paletteView: (view) => {
      ...
      if (view === "all")
        host.querySelector<HTMLInputElement>(".palette-search")?.focus({ preventScroll: true });
    },
    highlight: (code) => {
      ...
      if (code !== null && paletteIsOverlay(host)) {
        ...
        host
          .querySelector<HTMLElement>(
            store.get(state.editor$).highlightedColor ? ".highlight-clear" : ".palette-toggle",
          )
          ?.focus({ preventScroll: true });
      } else if (code === null)
    ...
    language: (locale) => {
      ...
      try {
        ...
        /* A preference failure must not interrupt editing. */
      }
    },
    ...
    create: (width, height) => {
      cancelImport();
      if (store.set(state.newDocument$, width, height)) created();
    },
    import: (file) => {
      cancelImport();
      importController = new AbortController();
      const signal = AbortSignal.any([lifetime.signal, importController.signal]);
      store.set(state.importCsv$, file, signal).then(
        (imported) => {
          if (imported && !signal.aborted) created();
        },
        (error) => {
    ...
    undo: () => store.set(state.undo$),
    redo: () => store.set(state.redo$),
    grid: () => store.set(state.toggleGrid$),
    codes: () => store.set(state.toggleCodes$),
    zoom: (factor) => {
      const area = editingArea(host);
      if (area)
        store.set(state.zoom$, factor, { x: area.x + area.width / 2, y: area.y + area.height / 2 });
    },
    fit,
    export: (options) => {
      ...
      exportPattern(model.document, model.title, options, signal).then(
        ({ blob, filename }) => {
  ...
  function shortcut(event: KeyboardEvent) {
    ...
    const paletteEscape =
      event.key === "Escape" && store.get(state.editor$).paletteOpen && paletteIsOverlay(host);
