...
export interface DraftStatus {
  kind: "automatic" | "recovered" | "recoveryFailed" | "saved" | "saveFailed" | null;
  detail?: unknown;
  action: "replace" | "retry" | null;
  error: boolean;
}
    ...
    grid.some(
      (row) =>
        ...
        row.some(
          (cell) =>
            cell !== null &&
            (typeof cell !== "string" || !Object.hasOwn(defaultPalette.colors, cell)),
        ),
    )
  ) {
...
export function createDrafts(storage: () => DraftStorage, notify: (status: DraftStatus) => void) {
  ...
  function flush() {
    ...
    try {
      storage().setItem(
        DRAFT_KEY,
        JSON.stringify({ version: 1, title: snapshot.title, grid: snapshot.grid }),
      );
  ...
  return {
    ...
    observe(snapshot: DraftDocument) {
      if (disposed) return;
      const unchanged =
        !previous || (snapshot.grid === previous.grid && snapshot.title === previous.title);
      previous = snapshot;
...
export function draftText(status: DraftStatus, t: Translate): string {
  ...
  if (!kind) return "";
  if (kind === "recoveryFailed")
    return t(($) => $.draft.recoveryFailed, { detail: errorText(status.detail, t) });
  return t(($) => $.draft[kind]);
}
