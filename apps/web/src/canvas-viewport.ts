export interface ViewArea { x: number; y: number; width: number; height: number }
export type CanvasEdge = "top" | "right" | "bottom" | "left";
export interface Obstruction extends ViewArea { edge: CanvasEdge }
...
export function unobscuredArea(canvas: ViewArea, panels: readonly Obstruction[]): ViewArea {
  ...
  for (const panel of panels) {
    const x = panel.x - canvas.x, y = panel.y - canvas.y;
    if (panel.width <= 0 || panel.height <= 0 || x >= canvas.width || y >= canvas.height ||
        x + panel.width <= 0 || y + panel.height <= 0) continue;
...
export function editingArea(host: HTMLElement): ViewArea | undefined {
  ...
  for (const panel of host.querySelectorAll<HTMLElement>("[data-canvas-panel]")) {
    const style = getComputedStyle(panel), edge = style.getPropertyValue("--canvas-edge").trim();
