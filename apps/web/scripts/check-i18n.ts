...
function placeholders(text: string): string {
  return [...text.matchAll(/\{\{([^{}]+)\}\}/g)].map(match => match[1].trim()).sort().join("|");
}
...
export function checkCatalogs(english: unknown, translated: unknown, locale: string): void {
  const source = flatten(english), target = flatten(translated);
  const expected = new Map<string, string>();
  for (const [key, text] of source) {
    if (key.endsWith("_one")) {
      if (!source.has(key.replace(/_one$/, "_other"))) throw new Error(`Missing English plural: ${key}`);
      continue;
    }
    const keys = key.endsWith("_other")
      ? new Intl.PluralRules(locale).resolvedOptions().pluralCategories.map(category => key.replace(/_other$/, `_${category}`))
      : [key];
  ...
  for (const [key, text] of expected) {
    ...
    if (placeholders(text) !== placeholders(translation)) throw new Error(`Mismatched ${locale} placeholders: ${key}`);
  }
  for (const key of target.keys()) if (!expected.has(key)) throw new Error(`Unexpected ${locale} translation: ${key}`);
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const read = (locale: string): unknown => JSON.parse(readFileSync(new URL(`../src/i18n/locales/${locale}.json`, import.meta.url), "utf8"));
