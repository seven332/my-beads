...
afterEach(() => {
  for (const { app, host } of mounts.splice(0)) { app.destroy(); host.remove(); }
...
it("rejects missing, extra, empty and mismatched translations while respecting Chinese plurals", () => {
  const source = { label: "Map {{source}}", beads_one: "{{count}} bead", beads_other: "{{count}} beads" };
  const valid = { label: "映射 {{source}}", beads_other: "{{count}} 颗" };
  expect(() => checkCatalogs(source, valid, "zh-CN")).not.toThrow();
  expect(() => checkCatalogs(source, { ...valid, label: "映射 {{code}}" }, "zh-CN")).toThrow("placeholders");
  ...
  expect(() => checkCatalogs(source, { label: valid.label, beads_one: "{{count}} 颗" }, "zh-CN")).toThrow("Missing");
});
...
it("switches the mounted UI and remembers the choice without changing the draft, history, Canvas or form settings", async () => {
  ...
  app.store.set(newDocument$, 3, 2); app.store.set(rename$, "My pattern 原图");
  ...
  expect(host.querySelector(".counts")?.textContent).toBe("1 颗 · 1 色");
  ...
  expect(reloaded.host.querySelector<HTMLSelectElement>(".language-picker select")!.value).toBe("zh-CN");
...
it("uses the browser language and keeps editing usable when preference storage is unavailable", async () => {
  vi.spyOn(navigator, "languages", "get").mockReturnValue(["zh-CN"]);
  const { app, host } = mount(() => { throw new Error("Denied"); });
...
it("retranslates existing validation errors and protects a corrupt draft during language changes", async () => {
  ...
  const { app, host } = mount(() => ({ getItem: key => values.get(key) ?? null, setItem: (key, value) => { values.set(key, value); } }));
  const text = "H7,H2\nH5";
  await app.store.set(importCsv$, { name: "invalid.csv", size: text.length, text: async () => text }, new AbortController().signal);
...
it("updates an open image dialog without discarding its preview or unfinished settings", async () => {
  ...
  await app.store.set(loadImage$, { name: "original.png", read: async () => ({ width: 1, height: 1, data: new Uint8ClampedArray([0, 0, 0, 255]) }) }, new AbortController().signal);
  await vi.waitFor(() => expect(host.querySelector("dialog")).not.toBeNull());
  const before = app.store.get(imageSession$), dialog = host.querySelector("dialog");
  const columns = dialog!.querySelector<HTMLInputElement>('[name="columns"]')!;
  columns.value = "7"; columns.dispatchEvent(new Event("input", { bubbles: true }));
  app.store.set(selectLocale$, "zh-CN");
  await vi.waitFor(() => expect(dialog!.textContent).toContain("导入像素图"));
  expect(host.querySelector("dialog")).toBe(dialog); expect(columns.value).toBe("7");
...
it("keeps locale-dependent errors isolated between stores", () => {
  const chinese = createStore(), english = createStore();
  chinese.set(selectLocale$, "zh-CN");
  chinese.set(newDocument$, 0, 1); english.set(newDocument$, 0, 1);
...
it("does not confuse non-Error rejections with cleared alerts", async () => {
  const store = createStore(); store.set(selectLocale$, "zh-CN");
  const signal = new AbortController().signal;
  await store.set(importCsv$, { name: "unreadable.csv", size: 0, text: () => Promise.reject(null) }, signal);
  expect(store.get(editor$).error).toBe("操作失败：null");
  await store.set(loadImage$, { name: "unreadable.png", read: () => Promise.reject(undefined) }, signal);
