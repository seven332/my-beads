...
import { loadImage$, imageSession$, updateImage$, overrideImage$, applyImage$, cancelImage$, changeImageSettings$ } from "../src/image-state.js";
import { newDocument$, editor$, beginStroke$, finishStroke$, importCsv$, rename$ } from "../src/state.js";
import type { RgbaImage } from "@my-beads/core";

const pixels: RgbaImage = { width: 2, height: 1, data: new Uint8Array([0, 0, 0, 255, 255, 255, 255, 255]) };
it("keeps Apply disabled when settings change during decoding", async () => {
  const store = createStore(), pending = Promise.withResolvers<RgbaImage>();
  const loading = store.set(loadImage$, { name: "Loading.png", read: () => pending.promise }, new AbortController().signal);
  store.set(changeImageSettings$); pending.resolve(pixels); await loading;
...
it("previews and overrides without changing the document until Apply", async () => {
  const store = createStore(), signal = new AbortController().signal;
...
it("rejects late decodes after edits, CSV, new images, cancellation or abort", async () => {
  for (const action of ["edit", "csv", "image", "cancel", "abort"] as const) {
    ...
    const loading = store.set(loadImage$, { name: "old.png", read: () => pending.promise }, controller.signal).catch(error => error);
    if (action === "edit") { store.set(beginStroke$, { x: 0, y: 0 }); store.set(finishStroke$); }
    if (action === "csv") await store.set(importCsv$, { name: "csv.csv", size: 2, text: async () => "H5" }, controller.signal);
    if (action === "image") await store.set(loadImage$, { name: "new.webp", read: async () => pixels }, controller.signal);
    if (action === "cancel") store.set(cancelImage$);
    if (action === "abort") controller.abort();
    pending.resolve(pixels); await loading;
