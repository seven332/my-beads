...
it("clips fast strokes at grid edges, including exit and re-entry", () => {
  ...
  expect(paintLine(grid, { x: 0, y: 0 }, { x: 20, y: 0 }, "H7"))
    .toEqual([["H7", "H7", "H7", "H7"], [null, null, null, null]]);
  expect(paintLine(grid, { x: -20, y: 1 }, { x: 20, y: 1 }, "H2"))
    .toEqual([[null, null, null, null], ["H2", "H2", "H2", "H2"]]);
});

it("fills only the four-connected region, including transparency and edges", () => {
  const grid = [[null, "H7", null], [null, "H7", "H7"], ["H7", null, null]];
  const result = floodFill(grid, { x: 0, y: 0 }, "H2");
  expect(result).toEqual([["H2", "H7", null], ["H2", "H7", "H7"], ["H7", null, null]]);
