...
RuleTester.describe = describe; RuleTester.it = it;
...
tester.run("lit-html UI copy", rules["no-hardcoded-ui-text"], {
  valid: [
    ...
    valid('html`<input placeholder=${t($ => $.search)} aria-label=${t($ => $.search)} .value=${userInput}>`;'),
    valid('html`<p>MARD 221 · ${count} · H7</p>`;'),
    valid('html`<p>&#35;4C4C40</p><p>CSV</p><p>→ ${count}%</p>`;'),
    valid('html`<!-- internal copy --><style>.save { color: red }</style><script>internal()</script>`;'),
    valid('html`<div id="save" data-label="internal" style="color:red" aria-labelledby="help" ${ref(node)}>${user.title}</div>`;'),
    ...
    valid('const items = [{ id: "save", text: t($ => $.save) }]; html`${items.map(item => html`<p>${item.text}</p>`)}`;'),
    valid('import { repeat } from "lit-html/directives/repeat.js"; const items = [["save", t($ => $.save)]]; html`${repeat(items, ([id]) => id, ([id, text]) => html`<p>${text}</p>`)}`;'),
  ...
  invalid: [
    ...
    invalid('const items = [{ id: "save", text: "Save" }]; html`${items.map(item => html`<p>${item.text}</p>`)}`;'),
    invalid('import { repeat } from "lit-html/directives/repeat.js"; const items = [["save", "Save"]]; html`${repeat(items, ([id]) => id, ([id, text]) => html`<p>${text}</p>`)}`;'),
    invalid('import { repeat as rows } from "lit-html/directives/repeat.js"; html`${rows(["Save"], text => text)}`;'),
    invalid('import { live as current } from "lit-html/directives/live.js"; html`<input placeholder=${current("Search")}>`;'),
    ...
    invalid('import * as lit from "lit-html"; const { html: template } = lit; template`<p>Save</p>`;'),
...
it("runs template syntax, binding and copy checks through the real web configuration", async () => {
  ...
  for (const [code, rule] of fixtures) {
    const [result] = await eslint.lintText(prefix + code, { filePath: "apps/web/src/lint-probe.ts" });
    assert.ok(result.messages.some(message => message.ruleId === rule && message.severity === 2), `${code}: ${JSON.stringify(result.messages)}`);
  }
  const [clean] = await eslint.lintText(prefix + 'html`<input type="number" min="1" .value=${live(value)} ?disabled=${busy} @input=${handle} aria-label=${t($ => $.size)}>`;', { filePath: "apps/web/src/lint-probe.ts" });
