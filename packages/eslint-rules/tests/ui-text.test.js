...
const options = [{ allowedLiterals: ["MARD 221", "CSV", "PNG", "H7", "Enter"],
  textFunctions: [{ module: "./image-view.js", name: "imagePicker", arguments: [0, 1] }] }];
const valid = code => ({ code: prefix + code, options });
const invalid = (code, count = 1) => ({ ...valid(code), errors: Array.from({ length: count }, () => ({ messageId: "literal" })) });

tester.run("no-hardcoded-ui-text", rules["no-hardcoded-ui-text"], {
  valid: [
    valid('h("button.primary", { attrs: { type: "submit", class: "primary" } }, t($ => $.export.download));'),
    valid('h("input", { attrs: { id: "palette-search", role: "searchbox", name: "query", "aria-describedby": "search-help", placeholder: t($ => $.palette.search) } });'),
    ...
    valid('const labels = { save: t($ => $.save), cancel: t($ => $.cancel) }; h("p", labels[action]);'),
    valid('const base = { save: t($ => $.save) }; const labels = { ...base }; h("p", labels[action]);'),
    valid('const base = { save: t($ => $.save) }; const labels = { ...base }; Object.entries(labels).map(([id, text]) => h("p", text));'),
    valid('const base = [["save", t($ => $.save)]]; const labels = [...base]; labels.map(([id, text]) => h("p", text));'),
    valid('let labels = { save: t($ => $.save) }; labels = { ...labels }; Object.values(labels).map(text => h("p", text));'),
    valid('let labels = [["save", t($ => $.save)]]; labels = [...labels]; labels.map(([id, text]) => h("p", text));'),
    valid('function button(label, name) { return h("button", { attrs: { name } }, label); } button(t($ => $.save), "save");'),
    valid('const entries = [{ id: "save", text: t($ => $.save) }]; entries.map(item => h("p", item.text));'),
    valid('const entries = [["save", t($ => $.save)]] as const; entries.map(([id, text]) => h("p", text));'),
    ...
    valid('import { imagePicker } from "./image-view.js"; function render(imagePicker) { imagePicker("not text"); }'),
    valid('const unused = "Internal value"; const value = node.getAttribute("title"); h("p", value);'),
    ...
    valid('let first = { save: t($ => $.save) }; let second = { ...first }; first = { ...second }; h("p", first.save);'),
    valid('element.setAttribute("data-id", "internal-identifier"); element.setAttribute("aria-labelledby", "dialog-heading");'),
    ...
    valid('const items = [{ id: "save", text: t($ => $.save) }]; const render = item => h("p", item.text); items.filter(allowed).map(render);'),
  ],
  invalid: [
    ...
    invalid('const base = { save: "Save" }; const labels = { ...base }; Object.entries(labels).map(([id, text]) => h("p", text));'),
    invalid('const base = { save: "Save" }; const labels = { ...base }; Object.values(labels).map(text => h("p", text));'),
    invalid('const base = { Save: t($ => $.save) }; const labels = { ...base }; Object.entries(labels).map(([text]) => h("p", text));'),
    invalid('const base = [["save", "Save"]]; const labels = [...base]; labels.map(([id, text]) => h("p", text));'),
    invalid('const base = [{ copy: "Save" }]; const labels = [...base]; labels.map(item => h("p", item.copy));'),
    invalid('const base = ["Save"]; const labels = [...base]; h("p", labels[index]);'),
    invalid('let labels = { save: "Save" }; labels = { ...labels }; Object.values(labels).map(text => h("p", text));'),
    invalid('let labels = [["save", "Save"]]; labels = [...labels]; labels.map(([id, text]) => h("p", text));'),
    invalid('const labels = { save: "Save", cancel: t($ => $.cancel) }; h("p", labels.save);'),
    invalid('const labels = { nested: { text: "Save" } }; const { nested: { text: copy } } = labels; h("p", copy);'),
    invalid('const labels = { save: "Save" }; const next = { ...labels }; h("p", next.save);'),
    invalid('let labels = { save: "Save" }; labels = { ...labels }; h("p", labels.save);'),
    invalid('let labels = { nested: { save: "Save" } }; labels = { ...labels }; h("p", labels.nested.save);'),
    invalid('let first = { save: "Save" }; let second = { ...first }; first = { ...second }; h("p", first.save);'),
    invalid('const labels = ["Save"]; h("p", labels[0]);'),
    invalid('const labels = [["save", "Save"]] as const; labels.map(([id, text]) => h("p", text));'),
    invalid('const choices = [{ id: "save", text: "Save" }]; choices.map(choice => h("p", choice.text));'),
    invalid('const labels = { save: "Save" }; Object.entries(labels).map(([id, text]) => h("p", text));'),
    ...
    invalid('function button(text) { return h("button", text); } const alias = button; alias("Save");'),
    ...
    invalid('import * as dom from "snabbdom"; dom["h"]("p", "Save");'),
    invalid('import { imagePicker as picker } from "./image-view.js"; picker("Open image", t($ => $.image.name), onFile);'),
    invalid('import * as images from "./image-view.js"; images.imagePicker(t($ => $.open), "Open image", onFile);'),
    ...
    invalid('const items = [{ id: "save", text: "Save" }]; const render = item => h("p", item.text); items.map(render);'),
    ...
    invalid('import * as images from "./image-view.js"; const { imagePicker: picker } = images; picker("Open image", t($ => $.open));'),
  ],
});
...
it("enforces the rule through the repository's real ESLint configuration without flagging export/test text", async () => {
  ...
  const [web] = await eslint.lintText(prefix + 'h("button", "Save");', { filePath: "apps/web/src/lint-probe.ts" });
  assert.deepEqual(web.messages.map(message => [message.ruleId, message.severity]), [[ruleId, 2]]);
  const [codes] = await eslint.lintText(prefix + 'h("p", ["H7", "B23", "MARD 221", "#4C4C40"]);', { filePath: "apps/web/src/lint-probe.ts" });
  assert.deepEqual(codes.messages, []);
  for (const filePath of ["packages/core/src/lint-probe.ts", "apps/cli/src/lint-probe.ts", "apps/web/tests/lint-probe.test.ts"]) {
    const [result] = await eslint.lintText('const chartTitle = "Printable chart";', { filePath });
    assert.equal(result.messages.some(message => message.ruleId === ruleId), false);
  }
});
