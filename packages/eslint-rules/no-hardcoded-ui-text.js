...
function patternPath(pattern, name) {
  ...
  const entries = pattern?.type === "ArrayPattern" ? pattern.elements.map((value, index) => [String(index), value])
    : pattern?.type === "ObjectPattern" ? pattern.properties.filter(p => p.type === "Property").map(p => [key(p), p.value]) : [];
...
export default {
  meta: {
    type: "problem",
    docs: { description: "Keep user-visible web copy in translation catalogs instead of string literals." },
    schema: [{ type: "object", additionalProperties: false, properties: {
      allowedLiterals: { type: "array", items: { type: "string" }, uniqueItems: true },
      textFunctions: { type: "array", items: { type: "object", additionalProperties: false,
        required: ["module", "name", "arguments"], properties: {
          module: { type: "string" }, name: { type: "string" },
          arguments: { type: "array", items: { type: "integer", minimum: 0 }, uniqueItems: true },
        } } },
    } }],
    messages: { literal: "Move UI text {{text}} to the language catalogs and use a typed translator." },
  },
  create(context) {
    ...
    function namespace(node, seen = new Set()) {
      ...
      if (def?.type === "ImportBinding" && def.node.type === "ImportNamespaceSpecifier") return def.parent.source.value;
    ...
    function imported(node, seen = new Set()) {
      ...
      if (def?.type === "ImportBinding" && def.node.type === "ImportSpecifier") {
        return { module: def.parent.source.value, name: def.node.imported.name ?? def.node.imported.value };
      }
      if (def?.type === "Variable") {
        const path = patternPath(def.node.id, node.name), module = namespace(def.node.init);
    ...
    function report(node, text) {
      ...
      if (!/\p{L}/u.test(text) || allowed.has(trimmed) || /^#[\da-f]{3}(?:[\da-f]{3})?$/i.test(trimmed)) return;
      if (!reported.has(node)) { reported.add(node); context.report({ node, messageId: "literal", data: { text: JSON.stringify(text) } }); }
    }
    function returnValues(fn) {
      ...
      if (fn.body.type === "BlockStatement") walk(fn.body); else values.push(fn.body);
    ...
    function defaults(pattern, name, seen) {
      if (pattern?.type === "AssignmentPattern") {
        ...
        return path === null ? [] : [...project(pattern.right, path, seen), ...defaults(pattern.left, name, seen)];
      }
      const children = pattern?.type === "ArrayPattern" ? pattern.elements
    ...
    function values(original, seen = new Set(), followParameters = true) {
      ...
      if (node.type === "Identifier") {
        ...
        return (binding?.defs ?? []).flatMap(def => {
          ...
          if (def.type === "Parameter") return followParameters ? parameterValues(def, node.name, seen) : [];
          if (def.type !== "Variable") return [];
          const path = patternPath(def.node.id, node.name) ?? [];
          const writes = binding.references.filter(ref => ref.writeExpr).map(ref => ref.writeExpr);
          const initial = [...new Set([def.node.init, ...writes].filter(Boolean))].flatMap(init => project(init, path, seen));
          return [...initial, ...defaults(def.node.id, node.name, seen)].flatMap(value => values(value, seen, followParameters));
        });
      }
      if (node.type === "MemberExpression") {
        const name = key(node);
      ...
      if (node.type === "CallExpression" && node.callee.type === "MemberExpression" && ["filter", "slice", "toSorted", "toReversed"].includes(key(node.callee))) {
    ...
    function inspect(original, seen = new Set()) {
      ...
      if (node.type === "Literal" && typeof node.value === "string") report(node, node.value);
      ...
      } else if (node.type === "ArrayExpression") node.elements.forEach(value => inspect(value, seen));
      else if (node.type === "SpreadElement") inspect(node.argument, seen);
      else if (node.type === "ConditionalExpression") { inspect(node.consequent, seen); inspect(node.alternate, seen); }
      else if (node.type === "LogicalExpression" || (node.type === "BinaryExpression" && node.operator === "+")) { inspect(node.left, seen); inspect(node.right, seen); }
        ...
        if (node.callee.type === "MemberExpression") {
          ...
          if (globalIdentifier(node.callee.object, "Object") && method === "values") mapItems(node, [], new Set()).forEach(value => inspect(value, seen));
          if (["map", "flatMap"].includes(method)) {
            for (const fn of values(node.arguments[0]).filter(value => functionTypes.has(value.type))) returnValues(fn).forEach(value => inspect(value, seen));
          }
          if (["join", "concat", "toUpperCase", "toLowerCase", "trim", "replace", "replaceAll"].includes(method)) {
    ...
    return {
      ...
      NewExpression: node => { if (["Error", "TypeError", "RangeError", "DOMException"].some(name => globalIdentifier(node.callee, name))) constructors.push(node.arguments[0]); },
      "Program:exit"() {
        for (const call of calls) {
          for (const fn of values(call.callee, new Set(), false).filter(value => functionTypes.has(value.type))) {
        ...
        for (const call of calls) {
          if (isH(call.callee)) {
            ...
            values(child).filter(value => value.type !== "ObjectExpression").forEach(value => inspect(value));
            if (values(call.arguments[0]).some(selector => selector.type === "Literal" && /^input(?:[.#]|$)/.test(selector.value))) {
              const types = ["attrs", "props"].flatMap(group => project(call.arguments[1], [group, "type"], new Set()));
              if (types.some(type => values(type).some(value => value.type === "Literal" && ["button", "submit", "reset"].includes(value.value)))) {
                for (const group of ["attrs", "props"]) project(call.arguments[1], [group, "value"], new Set()).forEach(value => inspect(value));
              }
            }
          }
          const spec = imported(call.callee);
          for (const helper of importedHelpers) if (spec?.module === helper.module && spec.name === helper.name) {
            for (const index of helper.arguments) inspect(call.arguments[index]);
          }
          if (["alert", "confirm", "prompt"].some(name => globalIdentifier(call.callee, name)) ||
              (call.callee.type === "MemberExpression" && ["fillText", "strokeText"].includes(key(call.callee)))) inspect(call.arguments[0]);
          if (call.callee.type === "MemberExpression") {
            const method = key(call.callee);
            if (["setAttribute", "setAttributeNS"].includes(method)) {
              const offset = method === "setAttributeNS" ? 1 : 0;
              if (values(call.arguments[offset]).some(value => value.type === "Literal" && textProperties.has(value.value))) inspect(call.arguments[offset + 1]);
            }
            if (globalIdentifier(call.callee.object, "window") && ["alert", "confirm", "prompt"].includes(method)) inspect(call.arguments[0]);
            if (globalIdentifier(call.callee.object, "document") && method === "createTextNode") inspect(call.arguments[0]);
          }
        }
      },
    };
