...
const rule = (description, create) => ({
  meta: { type: "problem", docs: { description }, schema: [], messages: { invalid: description } },
  create,
});
    ...
    kind(node) {
      ...
      while (scope) {
        ...
        if (variable) {
          ...
          return imported === "*" && callee.type === "MemberExpression"
            ? callee.computed
...
export const rules = {
  ...
  "signal-boundaries": rule(
    "Keep ccstate units dollar-suffixed, writable state private, and command graphs outside commands/views.",
    (context) => {
      ...
      return {
        ...
        CallExpression(node) {
          ...
          if (
            ...
            !node.parent.id.name.endsWith("$")
          )
            context.report({ node, messageId: "invalid" });
          if (kind === "state" && node.parent.parent?.parent?.type === "ExportNamedDeclaration")
            context.report({ node, messageId: "invalid" });
          for (let parent = node.parent; parent; parent = parent.parent) {
            if (units.kind(parent) === "command") {
              context.report({ node, messageId: "invalid" });
              break;
            }
          }
          if (/[/\\]view\.[jt]s$/.test(context.filename))
            context.report({ node, messageId: "invalid" });
        },
        ExportNamedDeclaration(node) {
          for (const specifier of node.specifiers) {
            const variable = context.sourceCode.getScope(node).set.get(specifier.local.name);
            if (variable?.defs.some((def) => units.kind(def.node.init) === "state"))
              context.report({ node: specifier, messageId: "invalid" });
          }
        },
  ...
  "accessor-scope": rule(
    "Call ccstate get/set directly in their callback; do not pass, alias, store or capture accessors.",
    (context) => {
      const units = primitives(context);
      return {
        ImportDeclaration: (node) => units.import(node),
        CallExpression(node) {
          ...
          if (param && kind === "command" && param.type !== "ObjectPattern") {
            context.report({ node: param, messageId: "invalid" });
            return;
          }
          const names =
            kind === "computed"
              ? [param?.name]
              : (param?.properties
                  ?.filter((p) => ["get", "set"].includes(p.key?.name))
                  .map((p) => p.value.name) ?? []);
          for (const variable of context.sourceCode.getDeclaredVariables(callback)) {
            ...
            for (const ref of variable.references) {
              ...
              while (ancestor && ancestor !== callback && !isFunction(ancestor))
                ancestor = ancestor.parent;
              if (!direct || ancestor !== callback)
                context.report({ node: id, messageId: "invalid" });
            }
          }
        },
      };
  ...
  "async-ownership": rule(
    "Async ccstate commands need a final AbortSignal and a cancellation check immediately after every await; lifecycle owns controllers.",
    (context) => {
      const units = primitives(context);
      function owner(node) {
        for (let parent = node.parent; parent; parent = parent.parent) {
          if (isFunction(parent))
            return units.kind(parent.parent) === "command" ? parent : undefined;
        }
      }
      return {
        ImportDeclaration: (node) => units.import(node),
        NewExpression(node) {
          if (node.callee.name === "AbortController" && !/[/\\]app\.ts$/.test(context.filename))
            context.report({ node, messageId: "invalid" });
        },
        CallExpression(node) {
          ...
          if (
            ...
            last.typeAnnotation?.typeAnnotation?.typeName?.name !== "AbortSignal"
          )
            context.report({ node, messageId: "invalid" });
        },
        AwaitExpression(node) {
          ...
          if (
            ...
            call.callee.object.name !== callback.params.at(-1)?.name ||
            call.callee.property.name !== "throwIfAborted"
          )
            context.report({ node, messageId: "invalid" });
        },
      };
